...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin, onApply, onNav }) => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full bg-white/95 backdrop-blur-md border-b border-slate-100 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <button onClick={() => handleNav('about', 'About Us')} className="text-sm font-semibold text-slate-600 hover:text-indigo-700 transition-colors">About Us</button>
            <button onClick={() => handleNav('benefits', 'Benefits')} className="text-sm font-semibold text-slate-600 hover:text-indigo-700 transition-colors">Benefits</button>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div className="text-center lg:text-left">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <button onClick={onApply} className="px-8 py-4 bg-indigo-700 text-white font-bold rounded-2xl text-lg shadow-xl shadow-indigo-100 flex items-center justify-center gap-2">Join the Society Now <ChevronRight size={20}/></button>
              <button onClick={() => handleNav('benefits', 'Benefits')} className="px-8 py-4 bg-white border border-slate-200 text-slate-900 font-bold rounded-2xl text-lg shadow-sm hover:bg-slate-50 transition-colors">Explore Benefits</button>
            </div>
          </div>
          <div className="relative">
             <div className="bg-indigo-600 rounded-[3rem] overflow-hidden shadow-2xl relative aspect-[4/3]">
                <img src="https://images.unsplash.com/photo-1573164060897-42571820373a?q=80&w=1469&auto=format&fit=crop" className="w-full h-full object-cover opacity-80 mix-blend-multiply" alt="Workforce" />
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {pillars.map((pillar, i) => (
              <div key={i} className="group">
                <div className="w-16 h-16 bg-slate-50 text-indigo-700 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-indigo-700 group-hover:text-white transition-all duration-300">
                  <pillar.icon size={32} />
                </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="text-center p-8 rounded-3xl bg-white/5 border border-white/10">
              <Users className="mx-auto mb-4 text-indigo-400" size={32} />
              <h4 className="text-4xl font-black mb-1">1,240</h4>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Active Members</p>
            </div>
            <div className="text-center p-8 rounded-3xl bg-white/5 border border-white/10">
              <Wallet className="mx-auto mb-4 text-indigo-400" size={32} />
              <h4 className="text-4xl font-black mb-1">₦500M+</h4>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Total Assets Managed</p>
            </div>
            <div className="text-center p-8 rounded-3xl bg-white/5 border border-white/10">
              <Handshake className="mx-auto mb-4 text-indigo-400" size={32} />
              <h4 className="text-4xl font-black mb-1">₦350M+</h4>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Loans Disbursed</p>
            </div>
            <div className="text-center p-8 rounded-3xl bg-white/5 border border-white/10">
              <BarChart3 className="mx-auto mb-4 text-indigo-400" size={32} />
              <h4 className="text-4xl font-black mb-1">10+ Yrs</h4>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Financial Excellence</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-20 items-center">
            ...
            <div className="lg:w-1/2 grid grid-cols-2 gap-4">
              <div className="space-y-4 pt-12">
                <div className="bg-indigo-700 aspect-square rounded-[2rem] p-8 flex flex-col justify-end text-white">
                  <Heart size={32} className="mb-4 text-indigo-300" />
                  <p className="font-black text-lg">Social Support</p>
                </div>
                <div className="bg-slate-900 aspect-[3/4] rounded-[2rem] p-8 flex flex-col justify-end text-white">
              ...
              <div className="space-y-4">
                <div className="bg-slate-100 aspect-[3/4] rounded-[2rem] p-8 flex flex-col justify-end text-slate-900">
                  <Users size={32} className="mb-4 text-indigo-700" />
                  <p className="font-black text-lg leading-tight">1,200+ Colleagues</p>
                </div>
                <div className="bg-white border border-slate-200 aspect-square rounded-[2rem] p-8 flex flex-col justify-end text-slate-900">
      ...
      {/* CTA Footer Section */}
      <section className="py-24 px-6 bg-white">
        <div className="max-w-5xl mx-auto bg-indigo-700 rounded-[3rem] p-12 lg:p-20 text-center text-white relative overflow-hidden shadow-2xl">
           ...
           <div className="relative z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                 <button onClick={onApply} className="px-10 py-5 bg-white text-indigo-700 font-black rounded-2xl hover:bg-indigo-50 transition-all shadow-xl flex items-center justify-center gap-2">Start Enrollment <ArrowRight size={20}/></button>
                 <button onClick={onLogin} className="px-10 py-5 bg-indigo-800/50 text-white font-black border border-indigo-400/30 rounded-2xl hover:bg-indigo-800 transition-all flex items-center justify-center gap-2">Member Login <Landmark size={20}/></button>
              </div>
           </div>
        </div>
      </section>
      ...
      <footer className="py-12 px-6 border-t border-slate-100 bg-slate-50/50">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
           ...
           <div className="flex gap-6">
              <button onClick={() => handleNav('about', 'Privacy Policy')} className="text-[10px] font-black uppercase text-slate-400 hover:text-indigo-700 tracking-widest">Privacy</button>
              <button onClick={() => handleNav('about', 'Terms of Service')} className="text-[10px] font-black uppercase text-slate-400 hover:text-indigo-700 tracking-widest">Terms</button>
           </div>
        </div>
      </footer>
    </div>
